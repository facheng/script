...
def convert_read_file(path, phone, start_char):
    with open(path, 'r') as files:
        ...
        for file in lines:
            ...
            cipher = {'cipher_no':file_str[file_str.find(flag,0,len(file_str))+len(flag):len(file_str)],'bindSource':1, 'smsyzm':'777777', 'mobileNum':phone,'rnd':0.8444005282553888}
...
def send_request(request_info):
    ...
    for requestData in request_info.payload:
        ...
        if not require_sms:
            response = requests.post(request_info.charge_url, data=requestData, verify=False, headers= requestHeaders)
            print response.text
        else:
            response = requests.post(request_info.send_code_url, data=requestData, verify=False, headers= requestHeaders)
            print response.text
            temp_json = response.text
            requestData['smsyzm']=temp_json[39:45]
            print '=== request =========',requestData
            response = requests.post(request_info.check_code_url, data=requestData, verify=False, headers= requestHeaders)
            print response.text
...
if __name__ == '__main__':
    ...
    payload = convert_read_file(path='text.txt',phone = phone, start_char= start_char) #path为钢镚密钥存放文件的路径，应与当前文件同一路径下
